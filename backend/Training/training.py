...
svc = pickle.load(open('backend\Training\Model\model.pkl', 'rb'))

# ========== Load Helper Data ==========
description = pd.read_csv("backend\Training\MasterData\description.csv")
precautions = pd.read_csv("backend\Training\MasterData\precautions_df.csv")
medications = pd.read_csv("backend\Training\MasterData\medications.csv")
workout = pd.read_csv("backend\Training\MasterData\workout_df.csv")
diets = pd.read_csv("backend\Training\MasterData\diets.csv")
...
diseases_list = {15: 'Fungal infection',
                 4: 'Allergy',
                 16: 'GERD', 
...
def get_predicted_value(patient_symptoms):
    ...
    predicted_disease = diseases_list[prediction_index]
...
if __name__ == "__main__":
    ...
    if invalid:
        print(f"\n These symptoms are not recognized: {invalid}")
        exit()

    # Warn if too few
    if len(user_symptoms) < 2:
        print("\n Please enter at least 2 or 3 symptoms for accurate prediction.")
        exit()

    predicted_disease, confidence = get_predicted_value(user_symptoms)

    if predicted_disease is None:
        print("\n Insufficient confidence in prediction. Please provide more symptoms.")
        exit()

    print("\n================= Predicted Disease ============")
    print(f"{predicted_disease} (Confidence: {round(confidence * 100, 2)}%)")
